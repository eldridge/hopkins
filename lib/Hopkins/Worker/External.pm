package # hide from the CPAN
	Hopkins::Worker::External;

use strict;

=head1 NAME

Hopkins::Worker::External - perl worker

=head1 DESCRIPTION

Hopkins::Worker::External represents the special case of a
worker that will be executing a system command.  it wraps
the execution in order to return results to hopkins.
...
sub _execute
{
	...
	if ($err) {
		print STDERR $err;
		$self->status->{error} = $err unless $self->status->{terminated};
		Hopkins->log_worker_failure($self->work->task->name, $err);
	}
}
